...
\begin{algorithmic}[1]
  ...
  \State $\vct{p}^* \gets \vct{p} - \frac{\delta t}{2} \nabla\phi(\vct{x}_n)\tr$ \Comment{Initial half momentum step $\hat{\vct{\psi}}_{\frac{\delta t}{2}}^{\textsc{a}}$.}
  \State $\vct{x}^* \gets \vct{x}_n + \delta t \mtx{M}^{-1}\vct{p}^*$ \Comment{$\hat{\vct{\psi}}_{\delta t}^{\textsc{b}}$.}
  \For{$s \in \fset{1 \dots L-1}$}
    \State $\vct{p}^* \gets \vct{p}^* - \delta t \nabla\phi(\vct{x}^*)\tr$ \Comment{$\hat{\vct{\psi}}_{\frac{\delta t}{2}}^{\textsc{a}} \circ \hat{\vct{\psi}}_{\frac{\delta t}{2}}^{\textsc{a}}$.}
    \State $\vct{x}^* \gets \vct{x}^* + \delta t \mtx{M}^{-1}\vct{p}^*$ \Comment{$\hat{\vct{\psi}}_{\delta t}^{\textsc{b}}$.}
  \EndFor
  \State $\vct{p}^* \gets \vct{p}^* - \frac{\delta t}{2} \nabla\phi(\vct{x}^*)\tr$ \Comment{Final half momentum step $\hat{\vct{\psi}}_{\frac{\delta t}{2}}^{\textsc{a}}$.} 
