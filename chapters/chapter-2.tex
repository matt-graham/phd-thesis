...
\begin{algorithmic}[1]
  ...
  \While{\textsc{True}}
    ...
    \EndIf
  \EndWhile
  \Function{LinearStepOut}{$\vct{x}_n,\,b_l,\,b_u,\,M$}
...
\small
\begin{algorithmic}[1]
  \State $h \sim \mathcal{U}\lpa 0,\utgtdens(\vct{x}_n) / \nrm{\vct{x}_n \gvn \vct{0}, \mtx{\Sigma}}\rpa$ \Comment{Sample slice height}
  \State $\vct{v} \sim \nrm{\vct{0}, \mtx{\Sigma}}$ \Comment{Sample vector setting slice ellipse}
  ...
  \While{\textsc{True}}
    \State $\vct{x}^* \gets \vct{x}_n \cos\theta + \vct{v} \sin\theta$  \Comment{Update proposed state}
    \If{$\utgtdens(\vct{x}^*) / \nrm{\vct{x}^* \gvn \vct{0}, \mtx{\Sigma}} \leq h$} \Comment{Proposed point not on slice}
