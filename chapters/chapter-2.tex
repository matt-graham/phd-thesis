...
\end{align}
In going from \eqref{eq:mh-det-proposal-derivation-2} to \eqref{eq:mh-det-proposal-derivation-3} we use a change of variables $\vct{y} = \vct{\phi}(\vct{x})$ in the integral. As $\vct{\phi}$ is an involution we have that $\vct{\phi}\circ\vct{\phi}(\vct{x}') = \vct{x}'$ and $\jacobianproddet{\vct{\phi}}\circ\vct{\phi}(\vct{x}') = \jacobproddet{\vct{\phi}}{\vct{x}'}^{-1}$ and so
...
%In the \ac{MCMC} methods discussed so far we have only required that we are able to evaluate a (unnormalised) density function for the target distribution of interest. For many distributions defined on real-values state spaes.
...
%A slice sampling algorithm is then used to find a $\theta$ value where the corresponding point on the ellipse lies within a slice $\set{S}_h$ defined by a further auxiliary draw $h$. Unlike the linear slice sampling algorithm, the slice is defined by the set of points whereby the ratio of the (unnormalised) target density $\utgtdens(\vct{x})$ to the Gaussian density $\nrm{\vct{x}\gvn\vct{0},\mtx{\Sigma}}$ is 


% The elliptical slice sampling algorithm 


% An intuition behind this choice is that the isopotential surfaces and so slices of a multivariate Gaussian distribution are ellipsoidal. If the target distribution is well approximated by a Gaussian distribution elliptical paths
...
\begin{subfigure}[b]{.32\linewidth}
\centering
\includetikz{global-local-latent-model-factor-graph}
\caption{Target $\tgtdens$}
\label{sfig:global-local-latent-model-factor-graph}
\end{subfigure}
\begin{subfigure}[b]{.32\linewidth}
\centering
\includetikz{global-local-latent-model-mean-field-q-factor-graph}
\caption{Mean-field $q$}
\label{sfig:global-local-latent-mean-field-q-factor-graph}
\end{subfigure}
\begin{subfigure}[b]{.32\linewidth}
\centering
\includetikz{global-local-latent-model-structured-q-factor-graph}
...
%Under this parametric assumption, rather than a variational optimisation problem we can now consider the variational objective functional $\varobj[q]$ as instead a function of the parameters $\ell(\vct{\theta}) = \varobj[f_{\vct{\theta}}]$. Typically the integrals involved in evaluating the parameteric variational objective $\ell(\vct{\theta})$ (and its gradients) cannot be solved analytically however . By using the sampling methods we will discuss in the next section of this chapter however it is possible estimate these integrals.
