...
A final crucial property of the Hamiltonian flow map is that it exhibits a time-reversal symmetry under negation of the momenta - if
\begin{equation}\label{eq:hamiltonian-dynamic-time-reversal-symmetry}
(\vct{x}',\vct{p}') = \vct{\psi}_t(\vct{x},\vct{p})
\iff
(\vct{x},-\vct{p}) = \vct{\psi}_t(\vct{x}',-\vct{p}').
\end{equation}
If we define $\vct{r} : \reals^D\times\reals^D \to \reals^D \times \reals^D$ as a `momentum-reversal' operator such that $\vct{r}(\vct{x},\vct{p}) = (\vct{x},-\vct{p})$ then this time-reversal symmetry means that the composition $\vct{r} \circ \vct{\psi}_t$ is an involution. Further as $\vct{r}$ also has Jacobian determinant one, then the composition $\vct{r} \circ \vct{\psi}_t$ also itself has a unit Jacobian determinant.

Using the previous result for the Metropolis--Hastings accept ratio for the special case of a deterministic proposal formed by an involution \eqref{eq:metropolis-hastings-transition-deterministic-proposal}, we have that a proposal generated by applying $\vct{r} \circ \vct{\psi}_t$ to the current state pair $(\vct{x},\vct{p})$ for any $t$ has an accept probability of one
\begin{align}\label{eq:exact-hmc-accept-probability}
  ...
  \min\lbr 
    ...
    {
    \exp\lpa
      -\hamiltonian \circ \vct{r} \circ \vct{\psi}_t(\vct{x}, \vct{p})
    \rpa
    }
    {
    \exp\lpa
    ...
    \left|\,\jacob{\vct{r} \circ \vct{\psi}_t}{\vct{x},\vct{p}}\right|
  \rbr\\
  &=
  \min\lbr 
    1,\,
    \exp\lpa
      \hamiltonian(\vct{x},\vct{p}) -\hamiltonian \circ \vct{r} \circ \vct{\psi}_t(\vct{x}, \vct{p})
    \rpa
  \rbr = 1.
\end{align}
...
\begin{align}\label{eq:leapgfrog-component-steps}
  \hat{\vct{\psi}}_{\delta t}^{\textsc{a}}(\vct{x},\vct{p}) = 
  \lpa\vct{x},\vct{p}-\delta t \nabla\phi(\vct{x})\tr\rpa,
  ~~
  \hat{\vct{\psi}}_{\delta t}^{\textsc{b}}(\vct{x},\vct{p}) = 
...
\end{equation}
Each leapfrog step is time-reversible and volume conserving. The composition of $L$ leapfrog steps $\lpa\hat{\vct{\psi}}^{\textsc{lf}}_{\delta t}\rpa^L$ maintains these properties. The alternative symmetric composition $\hat{\vct{\psi}}_{\frac{\delta t}{2}}^{\textsc{b}} \circ \hat{\vct{\psi}}_{\delta t}^{\textsc{b}} \circ \hat{\vct{\psi}}_{\frac{\delta t}{2}}^{\textsc{b}}$ also defines a symplectic integrator and is also often termed the leapfrog method. In practice when multiple leapfrog steps are composed together the intermediate half time-steps can be combined, for example using \eqref{eq:leapfrog-step-composition}
\begin{equation*}
  \lpa
  \hat{\vct{\psi}}_{\frac{\delta t}{2}}^{\textsc{a}} \circ 
  \hat{\vct{\psi}}_{\delta t}^{\textsc{b}} \circ 
  ...
  \hat{\vct{\psi}}_{\frac{\delta t}{2}}^{\textsc{a}} \circ 
  \hat{\vct{\psi}}_{\delta t}^{\textsc{b}} \circ 
  \hat{\vct{\psi}}_{\delta t}^{\textsc{a}} \circ
  \hat{\vct{\psi}}_{\delta t}^{\textsc{b}} \circ 
  \hat{\vct{\psi}}_{\frac{\delta t}{2}}^{\textsc{a}}
\end{equation*}
...
\begin{equation}\label{eq:hmc-accept-probability}
  ...
  \min\lbr 
    1,\,
    \exp\lpa
      \hamiltonian(\vct{x},\vct{p}) -\hamiltonian \circ \vct{r} \circ \lpa \hat{\vct{\psi}}^{\textsc{lf}}_{\delta t}\rpa^L(\vct{x}, \vct{p})
    \rpa
  \rbr.
\end{equation}
...
The function $\psi : \set{X} \to \reals$ defines a corresponding energy function for a base distribution $Q$ with \emph{normalised} density $q(\vct{x}) = \exp\lpa-\psi(\vct{x})\rpa$ with respect to $\mu$. The base distribution is typically chosen to have a simple unimodal density with mass covering as many of the regions of high density under the target density in $\set{X}$ as possible. When the target distribution corresponds to the posterior in a Bayesian inference task, $Q$ is often chosen as the prior distribution on the target variables which will typically have a simple unimodal form and be much more diffuse than the posterior. If the state space $\set{X}$ consists of a finite set of values, the base distribution can be chosen to be uniform across $\set{X}$ in which case $\psi(\vct{x})$ is constant and can be omitted from \eqref{eq:simulated-tempering-joint-target-density}.

Importantly the conditional distribution $\prob{\rvct{x}|\rvar{k}}$ on the target variables $\rvct{x}$ for $\rvar{k} = 0$ ($\beta_0 = 0$) corresponds to the base distribution $Q$ and to the target distribution $\tgtprob$ for $\rvar{k} = K$ ($\beta_K = 1$). We can therefore use the target variable components of sampled chain states for which $\rvar{k} = K$ to estimate expectations with respect to the target distribution $\tgtprob$. For intermediate values of $\rvar{k}$ the conditional distribution geometrically interpolates between $\tgtprob$ and $Q$. Figure \ref{fig:geometric-bridge-example} shows a simple example of this geometric bridging between a unimodal univariate base density and bimodal target density for $K=5$ inverse temperature values.
...
\end{equation}
As $\int_{\set{X}}\exp\lpa-\phi(\vct{x})\rpa\,\mu(\dr\vct{x})=Z$ and $\int_{\set{X}}\exp\lpa-\psi(\vct{x})\rpa\,\mu(\dr\vct{x})=1$ we have 
\begin{equation}
...
The relationship between the marginal density $\pden{\rvar{k}}$ and $Z$ although presenting challenges in terms of choosing the weights $\lbrace w_k \rbrace_{k=0}^K$ also however demonstrates that simulated tempering chains can be used to estimate $Z$. In particular we have that
\begin{equation}\label{eq:simulated-tempering-norm-const-marginal-density}
  Z = \exp(w_0 -w_K) \frac{\pden{\rvar{k}}(K)}{\pden{\rvar{k}}(0)}.
\end{equation}
Given the sampled states $\lbrace \vct{x}^{(s)}, k^{(s)} \rbrace_{s=1}^S$ of a simulated tempering chain, one way to form a consistent estimate of $Z$ is therefore to compute the ratio of the counts of samples with $\rvar{k}= K$ to those with $\rvar{k} = 0$,
...
Having now reviewed both the inference tasks and \ac{MCMC} methods underlying the work in this thesis, we are now in a position to more concretely outline the contributions made in the rest of the thesis.
