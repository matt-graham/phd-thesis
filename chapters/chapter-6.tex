...
\small
\begin{algorithmic}[1]
  \State $\vct{x}^* \sim q(\vct{x}_n)$ \Comment{Propose new target state.}
  ...
  \If{$ v <  \frac{q(\vct{x}_n\gvn\vct{x}^*)\,\hat{p}^*}{q(\vct{x}^*\gvn\vct{x}_n)\,\hat{p}_n}$}
...
\small
\begin{algorithmic}[1]
  \State $\vct{u}^* \sim \rho$ \Comment{$\transop_1$: MI update to auxiliary variables.}
  \State $v \sim \mathcal{U}(0,1)$
  ...
  \EndIf
  \State $\vct{x}^* \sim q(\vct{x}_n)$ \Comment{$\transop_2$: MH update to target variables.}
  \State $w \sim \mathcal{U}(0,1)$
  \If{$ w <  \frac{q(\vct{x}_n\gvn\vct{x}^*)\,\esttgtdens(\vct{x}^*\gvn\vct{u}_{n+1})}{q(\vct{x}^*\gvn\vct{x}_n)\,\esttgtdens(\vct{x}_n\gvn\vct{u}_{n+1})}$}
