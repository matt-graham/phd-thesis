...
\marginpar{If $\set{U}$ and \set{V} are an open subset of $\reals^M$ and $\reals^N$ respectively with $N > M$ and $\vctfunc{\phi} : \set{U} \to \set{V}$ is a differentiable map, then $\vctfunc{phi}$ is a \emph{submersion} if the Jacobian $\jacobian{\vctfunc{\phi}}$ is full row-rank everywhere in $\set{U}$.}   
