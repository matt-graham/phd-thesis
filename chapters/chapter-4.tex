...
%    \rho(\vct{u})
%  \,\dr\vct{u}.
\marginpar{The \emph{fibre} $\vctfunc{m}^{-1}(\vct{y})$ of an element $\vct{y} \in \set{Y}$ under a map $\vctfunc{m} : \set{X} \to \set{Y}$ is the pre-image of the singleton set $\lbrace \vct{y} \rbrace$ under $\vctfunc{m}$.}
...
In the parameterisation used in \eqref{eq:lotka-volterra}, all of the parameter variables $\rvct{z}$ are required to be positive. A simple choice of a prior distribution on the parameters is therefore a log-normal distribution
\begin{equation}
...
\EndFunction
\Function{$\genfunc_{\rvct{x}|\rvct{z}}$}{$[\rvct{z}_a;\,\rvct{z}_b;\,\rvct{z}_c]$}
