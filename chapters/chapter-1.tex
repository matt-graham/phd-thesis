...
\end{equation}
Correspondingly we denote the conditional probability of the event of the random variable $\rvar{x}$ taking a value in $\set{A} \in \sset{F}$ given the event that the random variable $\rvar{y}$ takes a value in $\set{B} \in \sset{G}$ as $\prob{\rvar{x}|\rvar{y}}(\set{A}\gvn\set{B})$. Using \eqref{eq:conditional-probability} and \eqref{eq:joint-probability-rvar}, $\prob{\rvar{x}|\rvar{y}}$ and $\prob{\rvar{x}|\rvar{y}}$ can be shown to satisfy
...
The two Lebesgue integral notations above are equivalent and we will use them interchangeably. The density function $f$ is also termed the \emph{Radon-Nikodym derivative} of $\nu$ with respect to $\mu$, denoted $\td{\nu}{\mu}$. Density functions therefore represent a convenient way to define a probability distribution with respect to an appropriate base measure. % (which the probability measure will be absolutely continuous with respect to).
...
Now let $\rvct{x}$ be a random vector taking values in the measurable space $(\set{X},\,\borel(\set{X}))$ and define $\rvct{y} = \vct{\phi} \circ \rvct{x}$ as a random vector taking values in  $(\set{Y},\,\borel(\set{Y}))$. If $\prob{\rvct{x}}$ has a density $\pden{\rvct{x}}$ with respect to the Lebesgue measure then we have that
\begin{equation}
\begin{split}
...
%  \nrm{\vct{a} \gvn m_a\mathbf{1}, s_a^2 \idmtx}
%  \nrm{\vct{b} \gvn m_b\mathbf{1}, s_b^2 \idmtx}\\
...
%  &\qquad
%  \iint
%    \nrm{\vct{n} \gvn \mu_m\vct{1}, \sigma_m^2\idmtx} \, 
%    \nrm{\vct{u} \gvn \mu_m\vct{1}, \sigma_m^2\idmtx}
%  \\[-1.5mm]
%  &\qquad\phantom{\iint}\,
%    \nrm{\vct{e}_1 \gvn -\vct{d}_1(\vct{x},\vct{y},\exp\vct{n},\exp\vct{u}),\sigma^2\idmtx}\,
%  \\
%  &\qquad\phantom{\iint}\,
%    \nrm{\vct{e}_2 \gvn -\vct{d}_2(\vct{x},\vct{y},\exp\vct{n},\exp\vct{u}),\sigma^2\idmtx}
%  \,\dr\vct{u}\,\dr\vct{n}
%\end{split}
%\end{equation}
