...
\end{subfigure}
\caption[Example of implicit probabilistic model.]{Simple example of a probabilistic model where the observed variable is a non-bijective function of two latent variables.}
...
\end{equation}
Inference is then typically posed as the problem of computing the posterior distribution (density). This is typically not available in a (normalised) closed form as evaluating the denominator in \eqref{eq:bayes-theorem-density}, $\pden{\rvct{x}}$, sometimes termed the \emph{model evidence}, requires integrating the joint density over the parameters
...
\begin{tikzpicture}
  ...
  \node[latent, below=0.4 of mi] (ti) {$\rvct{t}^{(i)}$} ;
  ...
  \node[const, above=0.4 of mi] (xi) {$\vct{x}^{(i)}$}; % 
  \node[const, above=0.4 of mi, xshift=12mm, yshift=-1mm] (yi) {$\vct{y}^{(i)}$}; % 
  ...
  \node[latent, below=0.4 of tj] (mj) {$\rvct{m}^{(j)}$} ;
  ...
  \node[latent, below=0.4 of mj] (xj) {$\rvct{x}^{(j)}$}; % 
  \node[latent, below=0.4 of mj, xshift=12mm] (yj) {$\rvct{y}^{(j)}$}; %
...
When introducing the proposed model (Figure \ref{fig:observing-dark-worlds-factor-graph} for the \emph{Observing Dark Worlds} problem we glossed over how the parameters $\sigma$, $\mu_m$, $\sigma_m$, $\mu_t$ and $\sigma_t$ were chosen, assuming for simplicity they had somehow been set to reasonable values. Rather than fixing the parameter the values, a more Bayesian approach would be instead to also treat these unknown parameters as further unobserved variables to infer.

In particular we can encode our beliefs about plausible values for the variables by setting prior distributions on each of the variables and then jointly conditioning on \emph{all} of the observed data together. In the \emph{Observing Dark Worlds} competition, two distinct sets of galaxy data were provided - a training set of $N_{\textrm{train}} = 300$ of cluster images in which the observed ellipticities, galaxy coordinates and halo positions were all provided; and a test set of $N_{\textrm{test}} = 120$ images where only the galaxy observed ellipticities and coordinates are provided. 

Assuming both the training and test data are \ac{iid} we can form a \emph{hierarchical model} for the problem which specifies a joint density across the observed and latent variables for all of the training and test set clusters as well as the global variables specifying the distributions on the halo masses, core radii and galaxy intrinsic ellipticities. A factor graph for the proposed hierarchical model is shown in Figure \ref{fig:odw-hierarchical-factor-graph}; compared to the factor graph in Figure \ref{fig:observing-dark-worlds-factor-graph} some of the detail in the model structure for each cluster has been hidden to make the higher level structure clearer and the vector notation introduced when discussing inference in the model has been used to remove the need for plates indexed across galaxies and halos. The galaxy positions which are all observed have also been removed for clarity.

%As an example of how prior distributions might be chosen in practice: 
The prior on the intrinsic ellipticity scale variable $\upsigma$ has been set to $\mathrm{Gamma}(2,0.1)$ this having positive support as required, and reflecting the known properties of the intrinsic ellipticities - that they have non-zero variability (with the Gamma density tending to zero as $\upsigma \to 0$) and that they are bounded to unit magnitude and hence the standard deviation would be expected to be significantly less than one. For the halo mass and core radii distribution variables weaker priors have been set that support a wide range of plausible distributions while still discouraging extreme values.

When making predictions using the hierarchical model, due to the joint dependence between all cluster images it is now necessary to calculate conditional expectations on all of the test cluster halo coordinates jointly given the observed data. Even considering only the halo position variables the expectations are being calculated over, the resulting integral is now over a space of several hundred dimensions, and once all of the additional unobserved variables defined in the overall joint density are included, the resulting integral is over several thousand variables. Given the discussed infeasibility of using quadrature to compute conditional expectations of the halo positions for even a single cluster images, clearly in this case the need for computational methods for inference which scale to high dimensionalities is even more stark.
...
\begin{tikzpicture}
  ...
  \node[obs, right=of zm, xshift=10mm] (x) {$\rvct{x}$} ; %
  \factor[left=of x, xshift=-7mm] {zm-x} {below:\small $\pden{\rvct{x}|\rvct{z}_m}$} {zm} {x} ; %
...
\end{tikzpicture}
%\vskip 5pt
\caption[Model comparison factor graph.]{Model comparison factor graph.} %
\label{fig:model-comparison-factor-graph}
\end{figure}

So far we have discussed inference on the unobserved variables in a fixed model. An important second level of inference is comparing between competing models for observed data. This can be treated consistently within the existing probabilistic framework we have discussed, however the computations involved in inference over models can pose further challenges beyond those already discussed in the previous section. We will first pose the model inference problem in general terms before returning to the \emph{Observing Dark Worlds} problem to give a concrete example.

The general set up will be that we have a finite set of $M$ models which we index with the variable $\rvar{m} \in \fset{1 \dots M}$. A common special case will be where $M=2$ and we are performing a comparison between a pair of models. All models share the same observed random vector $\rvct{x}$, and each model has a model specific vector of unobserved variables $\rvct{z}_m$
