...
\begin{tikzpicture}
  ...
  \factor[left=of a, xshift=-1mm] {sa_ma-a} 
    {$\nrm{\rvar{m}_{\rvar{a}}\vct{1},\rvar{s}_{\rvar{a}}^2\mtx{I}}$} {sa,ma} {a} ; %
  ...
  \factor[left=of b, xshift=-1mm] {sb_mb-b} 
    {$\nrm{\rvar{m}_{\rvar{b}}\vct{1},\rvar{s}_{\rvar{b}}^2\mtx{I}}$} {sb,mb} {b} ; %
...
\marginpar{A half-Cauchy density is defined as\\
\mbox{$\mathcal{C}_{\scriptscriptstyle \geq 0}(x \gvn \gamma) = \frac{2}{\uppi \gamma \lpa 1 + \frac{x^2}{\gamma^2} \rpa}$}\\[1mm]	
for $x \geq 0$ and 0 otherwise.
}
Each of the factors in Figure \ref{fig:hier-lin-regression-factor-graph} is labelled with a shorthand for a probability density function corresponding to the conditional or marginal density factor associated with the node. In the examples in Figure \ref{fig:hier-lin-regression-factor-graph}, $\nrm{\mu,\sigma^2}$ corresponds to a Gaussian density with mean $\mu$ and variance $\sigma^2$. Similarly $\nrm{\vct{\mu},\mtx{\Sigma}}$ corresponds to a multivariate Gaussian with mean $\vct{\mu}$ and covariance $\mtx{\Sigma}$ and $\mathcal{C}_{\geq 0}(\gamma)$ corresponds to a half-Cauchy density with scale parameter $\gamma$. The dependence of the factors on the value of the random variable the density is defined on is omitted in the factor labels for brevity.
...
\end{tikzpicture}
\vskip 0pt
\caption[Visualisation of reverse-mode automatic differentiation.]{Visualisation of applying reverse-mode automatic differentiation to the computation graph in Figure \ref{fig:gaussian-density-computation-graph} to calculate the derivatives of the negative log density of a univariate Gaussian distribution.}
...
To calculate the full Jacobian from a computation graph representing a function with $M$ inputs $\fset{\rvar{x}_i}_{i=1}^M$ and $N$ outputs $\fset{\rvar{y}_i}_{i=1}^N$, i.e. the $N \times M$ matrix $mtx{J}$ with entries $J_{i,j} = \pd{\rvar{y}_i}{\rvar{x}_j}$, we can do a single forward pass and $N$ reverse passes each time accumulating the derivatives of one output variable with respect to all inputs. This leads to an overall computational cost that is $\mathcal{O}(N)$ times the cost of a single (forward) function evalaution to evaluate the full Jacobian. Note that as each of the reverse passes can trivially be run in parallel (in addition to any parallelisation of the operations in the forward and reverse passes themselves), this $\mathcal{O}(N)$ factor in the operation count need not corresponds to an equivalent increase in compute time.

An alternative to reverse-mode accumulation is \emph{forward-mode accumulation} \citep{wengert1964simple}, which insteads accumulates partial derivatives with respect to a single input variable alongside a forward pass through the graph. In contrast to reverse-mode, this allows calculation of the partial derivatives of all output variables with respect to a single input variable at a computational cost that is a constant factor of the cost of the evaluation of the original expression in the forward pass. Forward-mode accumulation therefore allows evaluation of the Jacobian of a function with $M$ inputs and $N$ outputs at an overall computational cost that is $\mathcal{O}(M)$ times the cost of a single function evaluation. For functions with $M \gg N$, e.g. scalar valued functions of multiple inputs, reverse-mode accumulation is generally therefore signficantly more efficient at computing the Jacobian. Forward-mode accumulation is however useful for functions with $N \gg M$, and also has the advantage over reverse-mode accumulation of avoiding the requirement to store the values of intermediate variables from the forward pass for the reverse pass(es).

% static graph versus dynamic graph implementations
% side advantages of graph compilation, device agnosticism
% tensor based operations

The direct overlap in our notation to represent variable and operation nodes in computation graphs and that used to represent (random) variable nodes and deterministic factor nodes in our above presentation of factor graphs is intentional. Although often the operations associated with a deterministic factor node in a factor graph will be more complex than the operations usually represented by nodes in a computation graph, this is only a matter of granularity of reprensentation - fundamentally they perform the same role. Similarly while we often consider random variables as stochastic quantities, formally they are defined as measurable functions on the sample space, and likewise the variable nodes in a computation graph represent functional transforms of their ancestor nodes. 

Like directed graphical models, a directed factor graph naturally specifies a process for sampling from a generative model via ancestral sampling, with values for the random variables in the graph successively calculated in a forward pass consisting of a combination of deterministic and stochastic transformations of the values of parent variables. A computation graph likewise specifies a generative process (how to compute the expression outputs given inputs) computed via a forward pass through the graph with the main differences being here that the inputs to that process are assumed to be given rather than sampled from marginal densities and the intermediate operations are all deterministic.
...
\end{subfigure}%
\caption[Factor node reparameterisation.]{Full and partial reparameterisation transforms of a factor node  representing a conditional density factor $\pden{\rvct{x}|\rvct{z}}$. In full reparameterisation an additional random variable $\rvct{n}$ is introduced which (unconditionally) independent of $\rvct{x}$ such that the output of a deterministic transformation $\rvct{x} = \vct{h}(\rvct{n},\rvct{z})$ has the same conditional density given $\rvct{z}$ as the original factor. In partial reparameterisation the introduced random variable $\rvct{n}$ is instead conditionally dependent on $\rvct{z}$.
}
\label{fig:factor-node-reparameterisation}
\end{figure}

Although we originally motivated computation graphs as giving a graphical representation of `mathematical expressions', it should be noted that these expressions need not be composed of the standard arithmetic type operations we might typically think. Any computation, involving arbitarily complex 
...
\begin{tikzpicture}
  ...
  \factor[left=of ua] {pr-ua} {$\exp\mathcal{C}_{\geq 0}(1)$} {} {ua} ; %
  \factor[left=of ma] {pr-ma} {$\nrm{0,1}$} {} {ma} ; %
  \factor[left=of na] {pr-na} {$\nrm{\vct{0},\mtx{I}}$} {} {na} ; %
  \op[left=of a, xshift=-2mm] {sa_ma_na-a} 
    {$\rvar{m}_{\rvar{a}} + \rvar{s}_{\rvar{a}} \rvct{n}_{\rvar{a}}$} {sa,ma,na} {a} ; %
  ...
  \factor[left=of ub] {pr-ub} {$\exp\mathcal{C}_{\geq 0}(1)$} {} {ub} ; %
  \factor[left=of mb] {pr-mb} {$\nrm{0,1}$} {} {mb} ; %
  \factor[left=of nb] {pr-nb} {$\nrm{\vct{0},\mtx{I}}$} {} {nb} ; %
  \op[left=of b, xshift=-2mm] {sb_mb_nb-b} 
    {\tiny $\rvar{m}_{\rvar{b}} + \rvar{s}_{\rvar{b}}\rvct{n}_{\rvar{b}}$} {sb,mb,nb} {b} ; %
